...
topfolder_name = "lnq_mc$(num_sims)_L$(leader_idx)_$(Dates.now())"
...
for iter in sim_iters
    ...
    plot!(q1, title="")#, xaxis=[-2.5, 2.5], yaxis=[-2.5, 2.5], xticks=-2:1:2, yticks=-2:1:2)
    filepath = joinpath(folder_name, "$(ii)_silq_lnq_position_L$(leader_idx).pdf")
    # filename = string("silq_lnq_results_leader", leader_idx, "_3_position.pdf")
    savefig(q1, filepath)

    plot!(q8, title="")
    # filename = string("silq_lnq_results_leader", leader_idx, "_3_convergence.pdf")
    filepath = joinpath(folder_name, "$(ii)_silq_lnq_convergence_L$(leader_idx).pdf")
    savefig(q8, filepath)

    plot!(q9, title="")
    # filename = string("silq_lnq_results_leader", leader_idx, "_3_cost.pdf")
    filepath = joinpath(folder_name, "$(ii)_silq_lnq_cost_L$(leader_idx).pdf")
    savefig(q9, filepath)
...
filename = joinpath(topfolder_name, string("silq_lnq_mc$(num_sims)_convergence_L$(leader_idx).pdf"))
savefig(convergence_plot, filename)

plot!(convergence_histogram, title="", tickfontsize=16, fontsize=12, labelfontsize=14)
filename = joinpath(topfolder_name, string("silq_lnq_mc$(num_sims)_convhistogram_L$(leader_idx).pdf"))
savefig(convergence_histogram, filename)

plot!(d1, title="")
filename = joinpath(topfolder_name, string("silq_lnq_mc$(num_sims)_L$(leader_idx)_dist_to_origin.pdf"))
savefig(d1, filename)

plot!(d2, title="")
filename = joinpath(topfolder_name, string("silq_lnq_mc$(num_sims)_L$(leader_idx)_dist_to_agent.pdf"))
savefig(d2, filename)
...
println("giffying...")
gif(anim, "mc$(num_sims)_lnq_L$(leader_idx)_silqgames_animation.gif", fps = 5)
println("done")
...
for ss in sim_iters
    ...
    for t in 2:snapshot_freq:T
        ...
        pos2_filepath = joinpath(folder_name, "0$(jj)_lf_t$(t)_lnq_positions_detail_L$(leader_idx).pdf")
        prob1_filepath = joinpath(folder_name, "0$(jj)_lf_t$(t)_lnq_probs_P1_L$(leader_idx).pdf")
        prob2_filepath = joinpath(folder_name, "0$(jj)_lf_t$(t)_lnq_probs_P2_L$(leader_idx).pdf")
...
filename = joinpath(topfolder_name, string("lf_lnq_mc$(num_sims)_L$(leader_idx)_prob_P1.pdf"))
savefig(p5_unc, filename)

filename = joinpath(topfolder_name, string("lf_lnq_mc$(num_sims)_L$(leader_idx)_prob_P2.pdf"))
