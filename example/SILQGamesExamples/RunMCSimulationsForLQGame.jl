...
topfolder_name = "lq_mc$(num_sims)_L$(leader_idx)_$(Dates.now())"
...
for iter in sim_iters
    ...
    filepath = joinpath(folder_name, "$(ii)_silq_lq_position_L$(leader_idx).pdf")
    ...
    filepath = joinpath(folder_name, "$(ii)_silq_lq_convergence_L$(leader_idx).pdf")
    savefig(q8, filepath)

    plot!(q9, title="", xaxis=[-0.1, 1.1], xticks=[0, 1])
    # filename = string("silq_lq_results_leader", leader_idx, "_3_cost.pdf")
    filepath = joinpath(folder_name, "$(ii)_silq_lq_cost_L$(leader_idx).pdf")
    savefig(q9, filepath)
...
# Configure the convergence plot. For the LQ case, we should only need 1 iteration.
...
filename = joinpath(topfolder_name, string("silq_lq_mc$(num_sims)_convergence_L$(leader_idx).pdf"))
savefig(convergence_plot, filename)

plot!(convergence_histogram, title="", tickfontsize=16, fontsize=12, labelfontsize=14)
filename = joinpath(topfolder_name, string("silq_lq_mc$(num_sims)_convhistogram_L$(leader_idx).pdf"))
savefig(convergence_histogram, filename)

plot!(d1, title="")
filename = joinpath(topfolder_name, string("silq_lq_mc$(num_sims)_L$(leader_idx)_dist_to_origin.pdf"))
savefig(d1, filename)

plot!(d2, title="")
filename = joinpath(topfolder_name, string("silq_lq_mc$(num_sims)_L$(leader_idx)_dist_to_agent.pdf"))
savefig(d2, filename)
...
println("giffying...")
gif(anim, joinpath(topfolder_name, "mc$(num_sims)_lq_silqgames_animation.gif"), fps = 5)
println("done")
...
for ss in sim_iters
    ...
    for t in 2:snapshot_freq:T
        ...
        pos2_filepath = joinpath(folder_name, "0$(jj)_lf_t$(t)_lq_positions_detail_L$(leader_idx).pdf")
        prob1_filepath = joinpath(folder_name, "0$(jj)_lf_t$(t)_lq_probs_P1_L$(leader_idx).pdf")
        prob2_filepath = joinpath(folder_name, "0$(jj)_lf_t$(t)_lq_probs_P2_L$(leader_idx).pdf")
...
filename = joinpath(topfolder_name, string("lf_lq_mc$(num_sims)_L$(leader_idx)_prob_P1.pdf"))
savefig(p5_unc, filename)

filename = joinpath(topfolder_name, string("lf_lq_mc$(num_sims)_L$(leader_idx)_prob_P2.pdf"))
