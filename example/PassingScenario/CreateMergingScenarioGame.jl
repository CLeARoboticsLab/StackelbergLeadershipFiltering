...
const NUM_MERGING_SCENARIO_SUBCOSTS = 12
...
function create_merging_scenario_costs(cfg, si, w_p1, w_p2, goal_p1, goal_p2; large_number=1e6)
    ...
    stay_within_lanes_p1(si, x, us, t) = begin
        ...
        if dist_along_lane ≤ L₁ # separate lanes
            upper_bound = 0.
            lower_bound = -w
        else if dist_along_lane ≤ L₁ + L₂ # linear progression to smaller lane
            lin_width_at_dist = (dist_along_lane - L₁)/L₂
            upper_bound = lin_width_at_dist
            lower_bound = -lin_width_at_dist
