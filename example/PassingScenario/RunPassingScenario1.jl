...
pos_main_filepath = joinpath(folder_name, "LF_passing_scenario_main_L$(gt_leader_idx).pdf")
...
for t in iter1
    ...
    p5, p6 = make_probability_plots(times[1:T], t, probs[1:T])
    plot!(p5, title="")
    plot!(p6, title="")

    pos2_filepath = joinpath(folder_name, "0$(ii)_LF_passing_scenario_positions_detail_L$(gt_leader_idx).pdf")
    prob1_filepath = joinpath(folder_name, "0$(ii)_LF_passing_scenario_probs_P1_L$(gt_leader_idx).pdf")
    prob2_filepath = joinpath(folder_name, "0$(ii)_LF_passing_scenario_probs_P2_L$(gt_leader_idx).pdf")
