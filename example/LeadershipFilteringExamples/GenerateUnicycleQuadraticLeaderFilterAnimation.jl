...
for t in iter1
    ...
    pos2_filepath = joinpath(folder_name, "0$(ii)_lf_t$(t)_uq_positions_detail_L$(leader_idx).pdf")
    prob1_filepath = joinpath(folder_name, "0$(ii)_lf_t$(t)_uq_probs_P1_L$(leader_idx).pdf")
    prob2_filepath = joinpath(folder_name, "0$(ii)_lf_t$(t)_uq_probs_P2_L$(leader_idx).pdf")
...
anim = @animate for t in iter
    ...
    _, p_px, p_py, p_θ, p_v, _, _ = plot_states_and_controls(dyn, times[1:T], true_xs, us)
...
println("giffying...")
filename = joinpath(folder_name, "unicycle_quadratic_leadfilt.gif")
