...
function plot_leadership_filter_measurements(dyn::Dynamics, true_xs, zs; show_meas_annotation=nothing)
    ...
    # plot!(p1, est_xs[x2_idx, :], est_xs[y2_idx, :], label=L"\mathcal{A}_2 Estimate", color=:turquoise2)
    scatter!(p1, zs[x2_idx, :], zs[y2_idx, :], color=:blue, marker=:plus, ms=6, markerstrokewidth=0, label=label_p2)
    scatter!(p1, [x₁[x2_idx]], [x₁[y2_idx]], color=:blue, label="")#L"$\mathcal{A}_2$ Start")
...
function plot_leadership_filter_measurement_details(dyn::Dynamics, particle_leader_idxs_t, num_particles, particle_num_iterations_t, particle_traj_xs_t, true_xs, est_xs; transform_particle_fn=(xs)->xs, t=nothing, letter=nothing, include_all_labels=false, meas_xs=nothing)
    x₁ = true_xs[:, 1]
    ...
    if include_all_labels
        p2 = get_standard_plot(;columns=2, legendfontsize=24)
    ...
    plot!(p2, true_xs[x1_idx, :], true_xs[y1_idx, :], color=:black, linestyle=:dot, linewidth=3, label=p1_truth_label)
    ...
    plot!(p2, true_xs[x2_idx, :], true_xs[y2_idx, :], color=:black, linestyle=:dot, linewidth=3, label=p2_truth_label)
    ...
    for n in 1:num_particles
        ...
        # && include_all_labels
        label_1 = (!has_labeled_p1 && does_p1_lead) ? L"$h(\tilde{x}_{t}; H_t = \mathcal{A}_1)$" : ""
        label_2 = (!has_labeled_p2 && !does_p1_lead) ? L"$h(\tilde{x}_{t}; H_t = \mathcal{A}_2)$" : ""
    ...
    if !isnothing(t) && !isnothing(letter)
        ...
        if !isnothing(meas_xs)
            ...
            scatter!([1.8], [1.85], marker=:star, color=:yellow, ms=10, label=L"Measurement at $t$")
            ...
            plot!([true_xs[x1_idx, t]-boxsize, 1.3],
            ...
            if true_xs[y2_idx, t] > 1.5
                ...
                plot!([true_xs[x2_idx, t]-boxsize, 0.1], 
                      [true_xs[y2_idx, t]+boxsize, 1.55], color=:black, linestyle=:dot)
                plot!([true_xs[x2_idx, t]+boxsize, 1.0],
                      [true_xs[y2_idx, t]+boxsize, 1.55], color=:black, linestyle=:dot)
            end
            ...
            plot!(
                ...
                inset = (1, bbox(0.7, 0.0, 0.3, 0.3, :top, :left)),
                ticks = nothing,
                subplot=subplot_idx,
                bg_inside = :lightgrey,
                axis=false,
...
function make_probability_plots(times, probs; player_to_plot=nothing, t_idx=nothing, include_gt=nothing, stddevs=nothing)
    ...
    if !isnothing(t_idx)
        ...
        plot!(tickfontsize=30, fontsize=30, labelfontsize=34, ylabel="", title="Leadership Probability", titlefontsize=30)

        label_str = join(t_idx, ",")
        for idx in t_idx
            t = times[idx]
            plot!(plot, [t, t], [-0.05, 1.05], color=:black, linestyle=:dash, linewidth=8, label="")#"t=$(label_str)",)
