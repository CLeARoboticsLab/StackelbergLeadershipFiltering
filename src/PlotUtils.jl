...
function plot_convergence_and_costs(num_iters, threshold, conv_metrics, evaluated_costs)
    ...
    plot!(title=title8, yaxis=:log, xlabel="# Iterations", ylabel=L"$\left\parallel\mathbf{x}_{1:T}^{k+1} - \mathbf{x}_{1:T}^{k}\right\parallel_\infty$")#Max Abs. State Difference")
