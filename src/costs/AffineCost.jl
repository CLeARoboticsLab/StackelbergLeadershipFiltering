...
function add_control_cost!(c::AffineCost, other_player_idx, Rij; r=zeros(size(Rij, 1))::AbstractVector{Float64}, cr=1.::Float64)
    @assert size(Rij, 1) == size(Rij, 2) == size(r, 1)
    @assert size(cr) == ()
...
function compute_cost(c::AffineCost, time_range, xh::AbstractVector{Float64}, uhs::AbstractVector{<:AbstractVector{Float64}})
    ...
    total = (1/2.) * x' * c.Q * x + c.q' * x + c.cq + compute_control_cost(c, time_range, xh, uhs)
    return total
end

function compute_control_cost(c::AffineCost, time_range, xh::AbstractVector{Float64}, uhs::AbstractVector{<:AbstractVector{Float64}})
    num_players = length(uhs)
